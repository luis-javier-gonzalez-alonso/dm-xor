 ...
 *   4. Non-blocking map(): XOR encode for writes runs in a workqueue
 *      so the submitting thread is released immediately.
 */
...
/* Single workqueue for both encode (writes) and decode (reads). */
...
struct xor_io_tracker {
	...
	struct work_struct work;   /* encode (writes) or decode (reads) */
...
static void xor_write_worker(struct work_struct *work)
{
	...
	/* Submit all clones to backing devices */
	for (d = 0; d < t->dev_count; d++)
		submit_bio(t->clones[d]);
}

/* ------------------------------------------------------------------ */
/* Read worker — XOR-decode (process context)                          */
/* ------------------------------------------------------------------ */

static void xor_read_worker(struct work_struct *work)
{
	struct xor_io_tracker *t =
		container_of(work, struct xor_io_tracker, work);
	int s, d;
...
static void xor_end_io(struct bio *clone)
{
	...
	if (bio_data_dir(t->orig_bio) == READ &&
		...
		INIT_WORK(&t->work, xor_read_worker);
...
static int xor_map(struct dm_target *ti, struct bio *bio)
{
	...
	if (verbose)
		pr_info("[%s] map: op=%u sector=%llu size=%u bounce=%d\n",
			DM_MSG_PREFIX, (__force unsigned)
	op,
	...
	/* ---- Step 4: Encode + submit (writes) or submit (reads/thin) -- */
	if (needs_bounce && bio_data_dir(bio) == WRITE) {
		/* Defer XOR encode + submit to workqueue so the calling
		 * thread is not blocked on CPU-intensive XOR computation.
		 */
		INIT_WORK(&t->work, xor_write_worker);
		queue_work(xor_wq, &t->work);
	} else {
		/* Reads and thin-clone ops: submit immediately */
		for (d = 0; d < ctx->dev_count; d++)
			submit_bio(t->clones[d]);
	}
				     ...
				     iterate_devices_callout_fn fn, void *data)
{
	...
	/* Every backing device stores the full sector range (this is a
	 * mirror-like layout, not striped).
	 */
